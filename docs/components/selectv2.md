...
| 项目 | 值 |
|---|---|
| 创建导出 | `EU_CreateSelectV2` |
| 组件分类 | 表单/选择 |
| Python helper | `examples/python/new_emoji_ui.py` 中的 `create_selectv2` 或同类 helper |
...
| API | 说明 |
|---|---|
| `EU_CreateSelectV2` | 当前组件相关导出 |
| `EU_GetSelectV2Index` | 当前组件相关导出 |
| `EU_GetSelectV2MatchedCount` | 当前组件相关导出 |
...
| `EU_SetSelectV2VisibleCount` | 当前组件相关导出 |
...
hwnd = ui.create_window("✨ 虚拟选择器 示例", 240, 120, 860, 560)
root = ui.create_container(hwnd, 0, 0, 0, 820, 500)
# 请根据 `examples/python/new_emoji_ui.py` 中的 helper 创建 `SelectV2`。
...
优先运行对应完整测试文件，例如 `tests/python/test_selectv2_complete_components.py`。如果该组件被组合测试覆盖，请查看 `tests/python/test_*_complete_components.py`。

## 文档维护

如果 `SelectV2` 新增、删除、重命名或修改 API，必须同步更新本文件、`docs/components/README.md`、`docs/api-index.md`、`examples/python/new_emoji_ui.py` 和 `DLL命令/易语言DLL命令.md`。
