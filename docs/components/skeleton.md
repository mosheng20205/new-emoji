...
`Skeleton` 是 new_emoji 的 数据展示 组件。当前状态：**已完成**。

已补骨架行数、头像模板、加载切换、节流闪动动画、状态读回、Python 封装和独立中文 emoji 验证
...
| 组件分类 | 数据展示 |
| Python helper | `examples/python/new_emoji_ui.py` 中的 `create_skeleton` 或同类 helper |
...
| API | 说明 |
|---|---|
| `EU_CreateSkeleton` | 当前组件相关导出 |
| `EU_GetSkeletonLoading` | 当前组件相关导出 |
| `EU_GetSkeletonOptions` | 当前组件相关导出 |
| `EU_SetSkeletonAnimated` | 当前组件相关导出 |
| `EU_SetSkeletonLoading` | 当前组件相关导出 |
| `EU_SetSkeletonOptions` | 当前组件相关导出 |
| `EU_SetSkeletonRows` | 当前组件相关导出 |
...
# 请根据 `examples/python/new_emoji_ui.py` 中的 helper 创建 `Skeleton`。
...
如果 `Skeleton` 新增、删除、重命名或修改 API，必须同步更新本文件、`docs/components/README.md`、`docs/api-index.md`、`examples/python/new_emoji_ui.py` 和 `DLL命令/易语言DLL命令.md`。
