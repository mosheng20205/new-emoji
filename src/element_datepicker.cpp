...
std::wstring DatePicker::display_text() const {
    if (range_select && range_start > 0) {
        ...
        int sm = 0;
        int sd = 0;
        datepicker_from_value(range_start, &sy, &sm, &sd);
        wchar_t start_buf[32];
        if (date_format == 1) {
            swprintf_s(start_buf, L"%04d年%02d月%02d日", sy, sm, sd);
        } else {
            swprintf_s(start_buf, L"%04d-%02d-%02d", sy, sm, sd);
        }
        if (range_end <= 0) {
            return std::wstring(L"开始 ") + start_buf;
        }
        int ey = 0;
        int em = 0;
        int ed = 0;
        datepicker_from_value(range_end, &ey, &em, &ed);
        wchar_t end_buf[32];
        if (date_format == 1) {
            swprintf_s(end_buf, L"%04d年%02d月%02d日", ey, em, ed);
        } else {
            swprintf_s(end_buf, L"%04d-%02d-%02d", ey, em, ed);
        }
        return std::wstring(start_buf) + L" 至 " + end_buf;
    }
    if (!has_value) {
        return L"请选择日期";
    }
...
void DatePicker::paint(RenderContext& ctx) {
    ...
    draw_text(ctx, display_text(), style, fg,
              ...
              1.0f, DWRITE_FONT_WEIGHT_NORMAL, DWRITE_TEXT_ALIGNMENT_LEADING);
...
void DatePicker::paint_overlay(RenderContext& ctx) {
    ...
    wchar_t title[32];
    swprintf_s(title, L"%04d 年 %02d 月", year, month);
    ...
    float grid_top = (float)py + 70.0f;
    float footer_h = 42.0f;
    float grid_h = (float)ph - 82.0f - footer_h;
    float cell_h = grid_h / 6.0f;
