...
| 创建导出 | `EU_CreateEmpty` |
| 组件分类 | 数据展示 |
| Python helper | `tests/python/test_new_emoji.py` 中的 `create_empty` 或同类 helper |
...
# 请根据 `tests/python/test_new_emoji.py` 中的 helper 创建 `Empty`。
...
如果 `Empty` 新增、删除、重命名或修改 API，必须同步更新本文件、`docs/components/README.md`、`docs/api-index.md`、`tests/python/test_new_emoji.py` 和 `DLL命令/易语言DLL命令.md`。
