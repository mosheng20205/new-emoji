...
本组件已补齐 Element UI NavMenu 常用桌面端样式：横向默认、横向暗色/自定义色、纵向默认、纵向暗色/自定义色、多级子菜单、禁用项、分组标题、图标、链接项、侧栏展开/收起、纵向滚轮与滚动条、激活路径读回、完整状态读回、选择回调、Python 封装、易语言命令文档和独立中文 emoji 验证。
...
- 垂直菜单内容高于组件高度时会显示原生纵向滚动条，支持鼠标滚轮、拖动滚动条、点击轨道和键盘 PageUp/PageDown。
...
该测试覆盖横向/纵向、默认/暗色、多级、分组、禁用、链接元数据、收起侧栏、长纵向菜单滚动条、颜色 Set/Get、状态读回和中文 emoji 首屏显示。
