...
- 支持 DWM 圆角的系统会优先使用系统抗锯齿圆角；Win10 等旧系统回退到 per-pixel alpha 分层窗口，通过 `UpdateLayeredWindow(ULW_ALPHA)` 提交 32bpp PBGRA 位图，并按当前 DPI / 屏幕缩放换算逻辑半径。`enabled=0` 或 `radius<=0` 恢复矩形窗口。
