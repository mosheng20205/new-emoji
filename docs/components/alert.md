...
| 创建导出 | `EU_CreateAlert` |
| 组件分类 | 反馈/浮层 |
| Python helper | `examples/python/new_emoji_ui.py` 中的 `create_alert` 或同类 helper |
...
# 请根据 `examples/python/new_emoji_ui.py` 中的 helper 创建 `Alert`。
...
如果 `Alert` 新增、删除、重命名或修改 API，必须同步更新本文件、`docs/components/README.md`、`docs/api-index.md`、`examples/python/new_emoji_ui.py` 和 `DLL命令/易语言DLL命令.md`。
