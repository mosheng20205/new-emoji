...
已补齐 Element Plus 示例覆盖的样式：有默认值、无默认值、透明度开关、预设色、默认/中等/小型/超小四种尺寸，并保留面板、十六进制输入校验、状态读回、变化回调、Python 封装和中文 emoji gallery 演示。
...
- 鼠标点击主框打开/关闭面板，点击预设色选择颜色。
- `show_alpha=1` 时面板底部显示透明度条，拖动/点击后写入 alpha。
