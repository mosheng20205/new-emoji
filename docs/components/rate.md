...
| 创建导出 | `EU_CreateRate` |
| 组件分类 | 表单/选择 |
| Python helper | `tests/python/test_new_emoji.py` 中的 `create_rate` 或同类 helper |
...
# 请根据 `tests/python/test_new_emoji.py` 中的 helper 创建 `Rate`。
...
如果 `Rate` 新增、删除、重命名或修改 API，必须同步更新本文件、`docs/components/README.md`、`docs/api-index.md`、`tests/python/test_new_emoji.py` 和 `DLL命令/易语言DLL命令.md`。
