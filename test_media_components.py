...
def on_click(element_id):
    if element_id == g_update_id:
        ...
        ui.dll.EU_SetScrollbarRange(g_hwnd, g_v_scrollbar_id, 160, 36)
        ui.dll.EU_SetScrollbarValue(g_hwnd, g_v_scrollbar_id, 72)
        ui.dll.EU_SetScrollbarOptions(g_hwnd, g_h_scrollbar_id, 120, 45, 0, 0)
        ui.dll.EU_SetScrollbarValue(g_hwnd, g_h_scrollbar_id, 45)
        print(
            "[更新] 轮播当前页:",
            ...
            "竖向滚动:",
            ui.dll.EU_GetScrollbarValue(g_hwnd, g_v_scrollbar_id),
            "最大值:",
            ui.dll.EU_GetScrollbarMaxValue(g_hwnd, g_v_scrollbar_id),
        )
...
def main():
    global g_hwnd, g_carousel_id, g_upload_id, g_v_scrollbar_id, g_h_scrollbar_id, g_update_id, g_remove_id, g_options_id, g_image_id
    ...
    ui.create_text(hwnd, content_id, "🧭 自定义滚动条", 458, 260, 190, 28)
    ui.create_card(
        hwnd, content_id,
        "滚动内容区域",
        "这里模拟一个带自定义滚动条的内容区域。当前批次先完成 D2D 视觉和数值交互，后续可以继续接入真实内容滚动。",
        1,
        458, 296, 330, 154
    )
    g_v_scrollbar_id = ui.create_scrollbar(
        hwnd, content_id,
        35, 100, 1,
        800, 300, 24, 142
    )
    ui.set_scrollbar_wheel_step(hwnd, g_v_scrollbar_id, 16)
    g_h_scrollbar_id = ui.create_scrollbar(
        hwnd, content_id,
        18, 100, 0,
        468, 460, 300, 24
    )
    ui.set_scrollbar_wheel_step(hwnd, g_h_scrollbar_id, 12)
    assert ui.get_scrollbar_options(hwnd, g_v_scrollbar_id) == (35, 100, 20, 1, False, 16)
    assert ui.get_scrollbar_options(hwnd, g_h_scrollbar_id) == (18, 100, 20, 0, False, 12)
    ...
    ui.create_infobox(
        hwnd, content_id,
        "✅ 本批完善",
        "图片加载状态、轮播循环选项、上传队列状态、滚动条范围与页大小均已开放 API。",
        196, 582, 500, 42
    )
    ...
    print(
        ...
        f"VScroll={ui.get_scrollbar_options(hwnd, g_v_scrollbar_id)} "
        f"HScroll={ui.get_scrollbar_options(hwnd, g_h_scrollbar_id)}"
    )
